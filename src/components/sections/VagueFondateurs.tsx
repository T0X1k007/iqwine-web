...
type PlanChoice = "standard" | "pro" | "famille" | "undecided";
...
export default function VagueFondateurs() {
  ...
  const validate = useCallback((): string | null => {
    if (!name.trim())
      return t("Votre nom est requis.", "Your name is required.");
    if (!email.trim())
      return t("Votre courriel est requis.", "Your email is required.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email.trim())) {
      return t("Courriel invalide.", "Invalid email.");
    }
    if (!cellarSize)
      return t(
        "Sélectionnez la taille de votre cave.",
        "Select your cellar size.",
      );
    return null;
        ...
        <FadeInOnScroll delay={0.15} className="lg:col-span-5">
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <a
              href={APP_SIGNUP_URL}
              onClick={() =>
                track(ANALYTICS_EVENTS.SIGNUP_CLICK, { source: "founders" })
              }
            ...
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <h3 className="iq-h4 mb-2">
                  {t(
                    "Accès prioritaire fondateurs (optionnel)",
                    "Founder priority access (optional)",
                  )}
                </h3>
                ...
                <NoteField value={note} onChange={setNote} locale={locale} />
...
function PlanChoiceField({
  ...
  const t = (fr: string, en: string) => (locale === "fr" ? fr : en);
  const options: PlanChoice[] = ["standard", "pro", "famille", "undecided"];
  return (
    <div className="flex flex-col gap-2">
...
function NoteField({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {t(
          "Un mot sur votre cellier (optionnel)",
          "A word about your cellar (optional)",
        )}
      </label>
      <textarea
        id="founder-note"
...
function SuccessBanner({
  ...
  return (
    <div className="flex flex-col items-center text-center py-4">
      ...
      <h3 className="iq-h3 italic mb-3">{t("Bien reçu.", "Received.")}</h3>
